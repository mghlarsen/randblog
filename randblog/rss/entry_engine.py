...
def convert_old_entries(f, feed):
    from randblog.crawler.link_engine import Link
    from randblog.corpus.item_engine import CorpusItem, convert_old_corpus_item
    ...
    for entry_id, e in f.entries.items():
        ...
        entry.populate_from_dict_info(e.info)
        entry.save()
        if 'corpus_item' in e.info:
            entry.corpus_item = convert_old_corpus_item(entry, e)
            entry.save()
